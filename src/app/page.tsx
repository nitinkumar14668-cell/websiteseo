...
export default function App() {
  const [activeTab, setActiveTab] = useState<"analyze" | "compare" | "ranking">(
    "analyze",
  );
      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-extrabold tracking-tight mb-4 text-white">
            {activeTab === "analyze" &&
              "Website ka Live SEO Score Check Karein"}
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto">
            ...
            {activeTab === "ranking" &&
              "Check karein ki aapka website kisi specific keyword ke liye kaisa rank kar raha hai."}
          </p>
        </div>
          ...
          {activeTab === "analyze" ? (
            ...
            <form onSubmit={handleCompare} className="flex flex-col gap-4">
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <div className="hidden sm:flex items-center justify-center text-xs font-black text-slate-500 tracking-widest uppercase">
                  VS
                </div>
                <div className="relative flex-grow">
                  <Globe
        ...
        {activeTab === "ranking" && rankingData && (
          ...
          >
            <div className="bg-slate-900/50 p-8 rounded-2xl border border-slate-800 text-center relative overflow-hidden">
              ...
              <h2
                className="text-xl font-bold truncate text-white mb-1"
                title={url}
              >
              ...
              <div className="grid grid-cols-2 gap-4 text-left">
                <div className="bg-black/40 p-4 rounded-xl border border-slate-800 flex flex-col gap-1">
                  ...
                  <span
                    className={cn(
                      "text-lg font-bold",
                      rankingData.difficulty === "High"
                        ? "text-rose-400"
                        : rankingData.difficulty === "Medium"
                          ? "text-amber-400"
                          : "text-emerald-400",
                    )}
                  >
                    {rankingData.difficulty}
                  </span>
                </div>
                <div className="bg-black/40 p-4 rounded-xl border border-slate-800 flex flex-col gap-1">
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
  ...
  return (
    <div className={cn("space-y-6", compact ? "" : "max-w-4xl mx-auto")}>
      ...
      <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800 flex flex-col sm:flex-row items-center gap-6">
        <div
          className={cn(
            "flex-shrink-0 w-32 h-32 rounded-full border-8 flex items-center justify-center flex-col",
            scoreBorder,
            scoreBg,
          )}
        >
          <span
            className={cn("text-4xl font-black tracking-tighter", scoreColor)}
          >
            {aiAnalysis.score}
          </span>
        ...
        <div className="flex-grow space-y-2 text-center sm:text-left">
          <h2 className="text-xl font-bold truncate text-white" title={url}>
            {new URL(url).hostname}
          </h2>
          ...
          <ul className="space-y-3 text-sm">
            ...
            <li className="flex justify-between items-center border-b border-slate-800/50 pb-2">
              ...
              <span
                className={cn(
                  "font-medium",
                  rawMetrics.h1Count !== 1 ? "text-rose-400" : "text-white",
                )}
              >
                {rawMetrics.h1Count}
              </span>
              ...
              <div className="text-right">
                <div className="text-[11px] text-slate-500">
                  ...
                  <span className="font-semibold capitalize text-white">
                    {aiAnalysis.estimatedRankingMetrics.keywordDensity}
                  </span>
                </div>
                <div className="text-[11px] text-slate-500">
                  Content:{" "}
                  <span className="font-semibold capitalize text-white">
                    {aiAnalysis.estimatedRankingMetrics.contentQuality}
                  </span>
                </div>
              </div>
            </li>
        ...
        <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800">
          ...
          <ul className="space-y-3 text-sm">
            {aiAnalysis.strengths.map((strength, idx) => (
              <li
                key={idx}
                className="flex gap-2 items-start border-b border-slate-800/30 pb-2 last:border-0 last:pb-0"
              >
                <CheckCircle
                  size={14}
      ...
      <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xs uppercase tracking-widest text-slate-500 font-bold flex items-center gap-2">
            <ShieldAlert size={16} className="text-rose-500" /> SEO Errors &
            Fixes
          </h3>
          {aiAnalysis.errors.length > 0 && (
            <span className="px-2 py-1 bg-rose-500/10 text-rose-500 text-[10px] rounded-full font-bold">
              {aiAnalysis.errors.length < 10
                ? `0${aiAnalysis.errors.length}`
        ...
        {aiAnalysis.errors.length === 0 ? (
          <p className="text-emerald-400 text-sm font-medium p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-xl">
            Bohot badhiya! Koi badi error nahi mili.
          </p>
        ) : (
          <ul className="space-y-3 mt-4">
            {aiAnalysis.errors.map((err, idx) => (
              ...
              >
                <div
                  className={cn(
                    "mt-1 font-mono text-xs font-bold shrink-0",
                    err.impact === "High"
                      ? "text-rose-500"
                      : err.impact === "Medium"
                        ? "text-amber-500"
                ...
                <div className="w-full">
                  ...
                  <div className="mt-3 p-3 bg-black/40 border border-slate-800 rounded-lg">
                    <strong
                      className={cn(
                        "block text-[10px] uppercase font-bold mb-1 tracking-widest",
                        err.impact === "High"
                          ? "text-rose-400"
                          : err.impact === "Medium"
                            ? "text-amber-400"
